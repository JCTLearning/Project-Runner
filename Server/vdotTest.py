...
class vdot:
    ...
    def vdotMiles(self, runnerTime):
        ...
        for miles in db:
            ...
            x = x + 1
        """
        Sorting --  what need to happen here is that each file is deleted upon the script running, but we store the testDb.db in the user fiie. Lets also name it mile or km depending on value.
        """
        ...
        conn = lite.connect('testDb.db')
        c = conn.cursor()
        c.execute('select vdot, time from data sort by time asc')
        returnSql = c.fetchall()


        vdot = returnSql[0] #selects the first value
        return vdot #this is the vdot for that number
